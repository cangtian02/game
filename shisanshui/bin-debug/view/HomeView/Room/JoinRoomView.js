...
var JoinRoomView = (function (_super) {
    ...
    function JoinRoomView(modal) {
        var _this = _super.call(this) || this;
        _this.modal = modal;
        // 主要场景盒子
        _this.enterBox = new egret.Sprite();
        // 按键盒子
        _this.keyBox = new egret.Sprite();
        // 提示语盒子
        _this.promptBox = new egret.Sprite();
        // 房号盒子
    ...
    JoinRoomView.prototype.createView = function () {
        // title
    ...
    JoinRoomView.prototype.createInputView = function () {
        // 房号主场景盒子
    ...
    JoinRoomView.prototype.createPromptView = function (text) {
        // 如果已经有绘制就先删除
    ...
    JoinRoomView.prototype.createKeyView = function () {
        // 按键主要场景
    ...
    JoinRoomView.prototype.keyTouch = function (evt) {
        // 可点击区域的X Y轴坐标
        var keyBox_X = (this.stage.stageWidth - this.keyBox.width) / 2;
        var keyBox_Y = this.modalWarp.y + this.enterBox.y + this.keyBox.y;
        // 点击坐标
        var evtX = evt.stageX;
        var evtY = evt.stageY;
        // 根据点击坐标得出点击的是哪个按键
     ...
     */
    JoinRoomView.prototype.createInputNumView = function (i, f) {
        // 输入房号
        ...
        if (f == 2 && this.roomNum.length > 0) {
            ...
            // 清空已绘制房号
            for (var j = this.roomBox.$children.length - 1; j > -1; j--) {
     ...
     * n 房号
     */
    JoinRoomView.prototype.jojnRoomFun = function (n) {
        var _this = this;
        this.modal.$parent.removeChild(this.modal);
        var self = HomeView.self;
        var param = '{"gid":' + Main.Game_GID + ',"rno":' + n + '}';
        httpAjax(true, Main.DEFAULT_CONFIG.config.met_getRoomByRno, param, function (event) {
